...
N = list(_ for _ in range(0, 10))
check = [0]*10
ans = []

def ineq_check(num, inequality):
    if inequality == "<":
        num = num+1
    else :
        num = num-1
...
def solve_max():
    ...
    for j in ineq:
        next_num = ineq_check(i, j)
        if 0 <= next_num < 10 and check[next_num] == 0:
            ans.append(next_num)
            check[next_num] = 1
            i = next_num
